...
for app in mgr.applications.list_all().await? {
    // `Application` is an enum tagged by `module` (one variant per IVR module):
    println!("{app:?}");
}
```

In Rust, `Application` is a `#[serde(tag = "module")]` enum — match on the variant to read
...
```rust
let copy = mgr.applications.clone(&app_id).await?;
