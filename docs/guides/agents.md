...
```rust
use babelforce_manager_sdk::ListAgentsQuery;
...
and the group/tag restrictions `groupIds`, `groups`, `tags` (Go currently exposes `groupIds`).
Rust's `list_all` collects every page unfiltered; `list` and `list_page` take a `ListAgentsQuery`.
