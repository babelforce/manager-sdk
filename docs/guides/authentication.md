...
// 2. Exchange the code via the OAuth token endpoint, then connect with the access token.
...
:::note
Rust resolves the token once at `connect` (no background refresh). For a long-lived session, refresh
manually with `mgr.auth.token("refresh_token", …)` (refresh tokens rotate — always present the newest
one), then rebuild the client. `Auth::RefreshToken { refresh_token, client_id }` also accepts a
refresh token and resolves it once at connect.
:::
...
request. In **TypeScript** and **Go** the token is refreshed transparently before it expires; in
**Rust** it is resolved once, when you `connect()`.
...
integrations. In **TypeScript** and **Go** the token refreshes transparently; in **Rust** it is
resolved once, when you `connect()`.
...
```rust
let mgr = ManagerClient::connect(Auth::Password { user, pass }).await?;
```
